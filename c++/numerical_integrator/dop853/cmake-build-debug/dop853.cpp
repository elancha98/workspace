...
int Dop853::dop853(FcnEqDiff fcn, double x, double xend, vector<double> &y, const vector<double> &rtoler,
                   const vector<double> &atoler, SolTrait solout, unsigned nrdens, const vector<unsigned> &icont,
                   int iout, ostream *fileout, double uround, double safe, double fac1, double fac2, double beta,
                   double hmax, double h, long nmax, long nstiff) {
    unsigned n = y.size();
    /* ====== VERIFY INPUT ====== */
    if (n == UINT_MAX)
    ...
    return dopcore(fcn, x, y, xend, hmax, h, rtoler, atoler, fileout, solout, iout, nmax,
                  uround, nstiff, safe, beta, fac1, fac2, icont, nrdens);
}

int
Dop853::dopcore(FcnEqDiff fcn, double x, vector<double> &y, double xend, double hmax, double h,
                const vector<double> &rtoler, const vector<double> &atoler, ostream *fileout, SolTrait solout,
                int iout, long nmax, double uround, long nstiff, double safe, double beta, double fac1, double fac2,
                const vector<unsigned> &icont, unsigned nrds) {
    unsigned n = y.size();
    ...
    /* === INITIALIZE INDIR === */ //TODO
    vector<unsigned> indir(n);
    if (nrds != n) {
        for (unsigned i = 0; i < n; i++)
            indir[i] = UINT_MAX;
        for (unsigned i = 0; i < nrds; i++)
            indir[icont[i]] = i;
    }
    ...
    if (iout) {
        hout = 1.0;
        xout = x;
        function<double(unsigned, double)> contd8 = [=](unsigned ii,  double x) {
            unsigned i = UINT_MAX;
            if (indir.size() == 0)
                i = ii;
            else
            ...
            double s = (x - xold) / hout;
            double s1 = 1.0 - s;

            return rcont1[i] + s * (rcont2[i] + s1 * (rcont3[i] + s * (rcont4[i] + s1 * (rcont5[i] + s * (rcont6[i] +
                                                                                                          s1 * (rcont7[i] + s * rcont8[i]))))));
        };
        bool irtrn = solout(naccpt + 1, xold, x, y, contd8);
        if (!irtrn) {
            if (fileout)
...
double Dop853::hinit(FcnEqDiff fcn, double x, const vector<double> &y, double posneg, const vector<double> &f0,
