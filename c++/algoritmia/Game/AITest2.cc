...
struct PLAYER_NAME : public Player {
	...
	void move(int id) {
		...
		if (!paths[id].empty()) {
			Pos d = paths[id].top();
			paths[id].pop();
			return execute(Command(id, dir(u.pos, d)));
		}

		stack<Pos> path_to_enemy = get<0>(a_star(u.pos, enemy_target, h0));
		Pos d = path_to_enemy.top();
		if (cost(cell(d).type) == 0) {
			execute(Command(id, dir(u.pos, d)));
		}
	}
