...
%%% on Tue Mar 13 21:42:00 2018
%%% for ernesto,,, (ernesto@ernesto-debian-pc)
...
\begin{defi}
    Sea $ \E $ un $ \k $-ev.  de dimensión $ n $, sea $ f \in T_p(\E) $
    y $ s \in\mathcal{S}_p $, entonces, definimos $ (\underline{s}f) \in
    T_p(\E) $ como
    \[
...
\end{defi}
\begin{example}
    Sea $ \E = \real^4 $, $ B = \setb{e_1, e_2, e_3, e_4} $, $ f = e_1^*
    \otimes e_2^* \otimes e_3^* \in T_3(\E) $ y $ s = (1,2,3) \in
    \mathcal{S}_3 $, entonces
    \[
...
\begin{teo*}
    Sea $ \E $ un $ \k $-e.v. de $ \dim n $, sea $ B=\left\{ e_1, \dots
    , e_n\right\} $ y sea $ p\leq n $.
    \begin{enumerate}[i)]
    \item
        \label{teo_it1} $ \dim A_p\left( \E \right) = \binom{n}{p}. $
    \item
        \label{teo_it2} Una base de $ A_p $ es $ \tilde{B} = \left\{e_{i_1}^*
        \wedge \cdots \wedge e_{i_p}^* \right\} $, $ i\leq i_1 < \cdots
        < i_p \leq n $.
    \item
        Si $ w \in A_p\left( \E \right) $, las coordenadas de $ w $ en
        la base anterior son:
        \[
            \left( w \left( e_{i_1}, \dots , e_{i_p} \right) \right)_{1\leq
            i_1 < \cdots < i_p \leq n}
        \]
\end{enumerate}
\end{teo*}
